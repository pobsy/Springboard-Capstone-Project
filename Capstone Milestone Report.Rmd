...
Carrying out a brief summary on these features in both data sets:


```{r}
describe.by(lc1$fico_range_low, lc1$dti)
describe.by(lc1$dti)
describe.by(lc1$loan_amnt)
describe.by(lc1$emp_length)
```
**************The brief comparative analysis here shows that the min and max values between the corresponding features in the data sets are similar - nothing stands out as being higher/lower than the other. This may also be a limitation but further analysis should show a difference.**************
...
The inital exploration has been useful to understand the interesting trends in the data and to give an indication of what will be worth exploring in more detail. There is a slight shift from the original plan in the Capstone proposal however the main goal of the project remains the same - to create a predictive model which can improve the screening process for loan applicants in order to reduce the risk to investors.
